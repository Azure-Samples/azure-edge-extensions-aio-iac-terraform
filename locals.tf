...
locals {
  ...
  aio_sp_object_id     = coalesce(var.bootstrap_aio_sp_object_id, module.bootstrap_service_principal[0].aio_sp_object_id)
  aio_sp_client_id     = coalesce(var.bootstrap_aio_sp_client_id, module.bootstrap_service_principal[0].aio_sp_client_id)
  aio_sp_secret        = coalesce(var.bootstrap_aio_sp_application_password, module.bootstrap_service_principal[0].aio_sp_application_password)
...
locals {
  server_setup_params = {
    ...
    aio_sp_client_secret         = local.aio_sp_secret
  }
