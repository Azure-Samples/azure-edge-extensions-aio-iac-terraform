...
resource "azurerm_arc_kubernetes_cluster_extension" "akri" {
  ...
  configuration_settings = {
    ...
    "agent.host.containerRuntimeSocket": var.aio_akri_container_runtime_socket,
    "kubernetesDistro": var.aio_akri_kubernetes_distro
  }
}
...
resource "azurerm_arc_kubernetes_cluster_extension" "opc_ua_broker" {
  ...
  depends_on = [
    ...
    azurerm_arc_kubernetes_cluster_extension.mq,
    azapi_resource.mq,
    azapi_resource.aio_targets_opc_ua_broker
  ]
